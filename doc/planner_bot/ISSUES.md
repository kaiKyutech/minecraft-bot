...
**問題**: GOAP目標実行中に途中で止める方法がない。

**現状**:
- GOAPプランニングが開始されると、成功または失敗まで止められない
- 長時間かかるタスクを中断したい場合に対応できない
- ボットが意図しない動作をしている時に停止できない

**必要な機能**:
- `!stop` または `!cancel` コマンドでGOAP実行を中断
- 実行中のスキルを安全に停止
- 途中で生成したアイテムなどの状態は保持
...
**動作イメージ**:
1. ユーザーが `!stop` を送信
2. 現在実行中のスキルを完了させる（途中のブロック掘削など）
3. 次のアクションを実行せずにGOAPループを終了
4. 「目標実行を中断しました」とメッセージ表示

**実装時の検討事項**:
- スキル実行中の中断は安全か？（例: 移動中、掘削中）
- 途中状態の扱い（部分的にクラフトしたアイテムなど）
- AbortController または flag による制御
- Pathfinder の移動中断処理

**関連ファイル**:
- `planner_bot/src/commands/goal_command.js` - 中断フラグの実装
- `planner_bot/src/commands/index.js` - !stop コマンド追加
- `planner_bot/src/skills/*.js` - スキル実行中の中断対応
